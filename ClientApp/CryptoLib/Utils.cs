...
namespace CryptoLib
{
    public class Utils
    {
        ...
        static public byte[] GetSha512FromFileStream(FileStream inputFileStream)
        {
            var sha3_512 = new Sha3Digest(512);
            ...
            while((bytesRed = inputFileStream.Read(buffer, 0, buffer.Length))> 0)
            {
                sha3_512.BlockUpdate(buffer, 0, bytesRed);
            }
